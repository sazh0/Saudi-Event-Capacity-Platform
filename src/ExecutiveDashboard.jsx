...
/* ═══ MOBILE CARD — HTML-based card for mobile layout ═══ */
function MobileCard({ tp, secColor, isHighlighted }) {
  const isDef = tp.gap < 0
  const col = isDef ? T.deficit : T.surplus
  const gaugeR = 44, gaugeSW = 9
  const semiLen = Math.PI * gaugeR
  const clamped = Math.min(tp.pct, 100)
  const dashOff = semiLen * (1 - clamped / 100)
  const gUid = useMemo(() => `mg${++_gid}`, [])
  const arc = `M ${60 - gaugeR} 60 A ${gaugeR} ${gaugeR} 0 0 1 ${60 + gaugeR} 60`

  return (
    <div className={`exec-mobile-card${isHighlighted ? ' exec-mobile-card-hl' : ''}`}
      data-card-id={tp.id} style={{ '--sec-color': secColor }}>
      <div className="exec-mob-accent" style={{ background: secColor }} />
      <div className="exec-mob-head">
        <span className="exec-mob-title">{tp.label}</span>
        <span className={`exec-mob-badge ${isDef ? 'def' : 'sur'}`}>{isDef ? 'عجز' : 'فائض'}</span>
      </div>
      <div className="exec-mob-gauge-wrap">
        <svg viewBox="0 0 120 70" className="exec-mob-gauge-svg">
          <defs>
            <linearGradient id={`${gUid}-g`} x1="0%" y1="0%" x2="100%" y2="0%">
              <stop offset="0%" stopColor={col} stopOpacity="0.25" />
              <stop offset="40%" stopColor={col} stopOpacity="0.82" />
              <stop offset="100%" stopColor={col} />
            </linearGradient>
          </defs>
          <path d={arc} fill="none" stroke="rgba(65,64,66,0.06)" strokeWidth={gaugeSW} strokeLinecap="round" />
          {clamped > 0 && (
            <path d={arc} fill="none" stroke={`url(#${gUid}-g)`} strokeWidth={gaugeSW} strokeLinecap="round"
              strokeDasharray={semiLen} strokeDashoffset={dashOff} />
          )}
          <text x={60} y={52} textAnchor="middle" fill={col}
            fontSize={18} fontWeight={900} fontFamily="'Cairo','BahijTheSansArabic',sans-serif">{tp.pct}%</text>
          <text x={60} y={65} textAnchor="middle" fill={T.txtSub}
            fontSize={6.5} fontWeight={600} fontFamily="'BahijTheSansArabic',sans-serif">نسبة التغطية</text>
        </svg>
      </div>
      <div className="exec-mob-divider" />
      <div className="exec-mob-metrics">
        ...
        <div className="exec-mob-metric">
          <span className="exec-mob-metric-lbl">الفارق</span>
...
function MobileCardList({ sectionId, yearlyData, highlightedId }) {
  ...
  return (
    <div className="exec-mob-list">
      <div className="exec-mob-list-header">
        <span className="exec-mob-list-dot" style={{ background: sec.color }} />
        <span className="exec-mob-list-title">{sec.label}</span>
        <span className="exec-mob-list-count">{tps.length} نقطة</span>
      </div>
      <div className="exec-mob-list-grid">
        {tps.map(tp => (
          <MobileCard key={tp.id} tp={tp} secColor={sec.color} isHighlighted={highlightedId === tp.id} />
        ))}
      </div>
    </div>
  )
...
/* ═══ MAP CANVAS ═══ */
function MapCanvas({ selected, onSelect, onDeselect, sectionFilter, yearlyData, highlightedId, svgRef, isMobile }) {
  ...
  /* On mobile: tight viewBox around map only. On desktop: include side card panels */
  let vbX, vbW, vbH
  if (isMobile) {
    vbX = 0; vbW = 1000; vbH = MAP_BASE_H
  } else {
    vbH = computeViewBoxHeight(sectionFilter)
    const sideW = computeSideWidth(sectionFilter)
    vbX = -sideW; vbW = 1000 + sideW * 2
  }
  ...
  return (
    <svg ref={svgRef} viewBox={`${vbX} 0 ${vbW} ${vbH}`} className="exec-map-svg" onClick={onDeselect}>
      ...
      {/* Side card panels — desktop only */}
      {!isMobile && sectionFilter && <SideChartsLayer key={sectionFilter} sectionId={sectionFilter} yearlyData={yearlyData} highlightedId={highlightedId} />}
      {visible.map(pin => {
        ...
        const r = isMobile
          ? (isSelected ? 12 : isHovered ? 10 : 8)
          : (isSelected ? 8 : isHovered ? 7 : 5)
        return (
          <g key={pin.id} onClick={e => { e.stopPropagation(); onSelect(pin.id) }}
            onMouseEnter={() => !isMobile && setHovered(pin.id)} onMouseLeave={() => !isMobile && setHovered(null)}
            ...
            {/* Invisible touch target — mobile */}
            {isMobile && <circle cx={pin.x} cy={pin.y} r={28} fill="transparent" />}
            {status === 'deficit' && !isSelected && (
              <circle cx={pin.x} cy={pin.y} r={r + 5} fill="none" stroke={T.deficit} strokeWidth={isMobile ? 1.5 : 1} opacity={0.35}>
...
export default function ExecutiveDashboard() {
  ...
  /* ═══ Scroll to card — mobile: HTML cards / desktop: SVG cards ═══ */
  const scrollToCard = useCallback((pinId) => {
    requestAnimationFrame(() => {
      if (isMobile) {
        const el = document.querySelector(`.exec-mobile-card[data-card-id="${pinId}"]`)
    ...
    <div className="exec-root cl-root" style={{ direction: 'rtl', background: '#fefefe', minHeight: '100vh', color: T.txt }}>
      ...
      <div className="exec-map-container">
        <MapCanvas selected={selectedPin} onSelect={handleSelectPin} onDeselect={handleDeselect}
          sectionFilter={sectionFilter} yearlyData={yearlyData} highlightedId={highlightedCard} svgRef={svgRef} isMobile={isMobile} />
      </div>
      {/* Mobile card list — replaces SVG side cards */}
      {isMobile && sectionFilter && (
        <div className="exec-content">
          <MobileCardList sectionId={sectionFilter} yearlyData={yearlyData} highlightedId={highlightedCard} />
        </div>
      )}
